...
- Inline-SVG chart/concept diagrams skip mmdc — a small `*.chart.json`/`*.svg.json` data file feeds a reusable template in `diagrams/_lib/`, producing an SVG fragment (already using `var(--token)`) injected the same way. Templates must also emit **per-diagram-prefixed ids** (or avoid `id`/`clipPath`/gradient/marker ids entirely) to satisfy the no-duplicate-id rule.
...
- `theme-map.mjs` maps each sentinel hex → the guide CSS token: `#444804 → var(--accent)` *(Parts 2/3; → `var(--blue)` for the Zero-to-Hero pilot, which has no `--accent` — see below)*, `#222802 → var(--surface2)`, `#333803 → var(--border)`, `#555805 → currentColor`, etc. (Mermaid needs real colors at render time, so we render with sentinels then swap.)
...
- Re-running `scripts/build-diagrams.mjs` reproduces the embedded SVGs **byte-for-byte on the same render environment** (idempotent — no git churn for a given machine; Mermaid bakes Chrome-computed text geometry into the SVG, so output can vary across Chrome/font sets — treat the committed SVGs as the source of record and regenerate intentionally). The inline-SVG `_lib` templates are deterministic everywhere. Editing a `.mmd` updates only its diagram.
